      ...
      <Table>
        <TableHeader>
          <TableRow className="bg-accent text-base">
            {["No.", ...header, ""].map((head, i) => (
              <TableHead key={i}>{head.toUpperCase()}</TableHead>
            ))}
          </TableRow>
        </TableHeader>
        <TableBody>
          {currentThresholds.map((threshold, i) => {
            return (
              <TableRow key={i}>
                {[i + 1, ...threshold.slice(0, header.length)].map(
                  (head, i) => (
                    <TableHead key={i}>
                      <div className="flex gap-2">
                        {head}
                        {
                          subjectIcons.find(([key]) =>
                            String(head).toLowerCase().includes(key),
                          )?.[1]
                        }
                      </div>
                    </TableHead>
          ...
          {totalPages > 1 && (
            <PaginationItem>
              <PaginationLink
                ...
                className={page === totalPages ? "bg-accent" : ""}
              >
                {totalPages}
              </PaginationLink>
            </PaginationItem>
